...
void rpmsg_handler::register_on_rx_callback(void (*func)(char *payload, size_t length))
{
  _on_rx_callback_func = func;
}

bool rpmsg_handler::is_connected()
{
  return _connected;
}

void rpmsg_handler::set_disconnected()
{
  _connected = false;
}

void rpmsg_handler::poll()
{
...
int16_t rpmsg_handler::send(char const *payload, size_t length)
{
  if (!_connected) {
